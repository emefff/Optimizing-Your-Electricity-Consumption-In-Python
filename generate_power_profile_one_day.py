# -*- coding: utf-8 -*-
"""
Created on Mon Apr  4 10:16:05 2024
...
def create_time(hour, minute):
    """
    Returns a time in the form of a string "HH:MM". When single digit numbers occur
    in hour or minute, it converts it to two digits: create_time(2,4) --> returns "02:04"
...
def inject_power_value(time, device, randomization_percentage):
    """
    Injects power values into a running TOTAL_POWER_CURVE_PER_DAY. time is the 
    current time, device is a list like ["07:00", "09:00", coffemaker_profile_1], which
    means we start the profile coffemaker_profile_1 at 07:00 and keep it running 
    until 09:00. Whatever values are in the power profile will be added to 
    TOTAL_POWER_CURVE_PER_DAY. Power values are injected per minute, thus the max. length 
    per day is 1440 minutes. If a power profile ends before the timespan ends,
    nothing is injected. Another idea would be to repeat the power profile
    if the given timespan is too large for the given profile. In this case, other
    problems may arise, because some power profiles are not repeatable, a possbile 
    reason for this could be that a device needs more power at the beginning.  

    Parameters
    ----------
    time : TYPE int
        DESCRIPTION. actual time of day in minutes 0-1439
    device : TYPE list
        DESCRIPTION. list that contains start and end time, as well as a power profile.
    randomization_percentage : TYPE float or int
        DESCRIPTION. a percentage value 0-100% (very large values are not recommended),
                    that adds a randomization to the given power cuve in device.
                    The randomization is always value_profile*(1 + np.random.randn() * randomization_percentage/100).
                    Thus, when randomization_percentage is large, negative values may
                    occur. Use with caution.                    

    Returns
    -------
    None. Technically, it doesn't return anything but adds to global list variable
    TOTAL_POWER_CURVE_PER_DAY. 
...
def is_active(time, device):
    """
    Checks if a device is active at the given time.

    Parameters
    ----------
    time : TYPE int
        DESCRIPTION. Time of day in minutes 0-1439.
    device : TYPE list
        DESCRIPTION. list that contains start and end time, as well as a power profile.
    ...
    -------
    active : TYPE boolean
        DESCRIPTION. if device = active then active = True, otherwise it is not.
...
def total_energy_consumed_per_day(total_power_consumption_per_min): # negative power values are LOST!
    """
    Sums up all power values over one day. Discards negative values, they are lost
    energy. 
    

    Parameters
    ----------
    total_power_consumption_per_min : TYPE list of floats
        DESCRIPTION. List contains power values per minute of day.

    Returns
    -------
    sum_energy_per_day_kWh : TYPE float
        DESCRIPTION. Calculated kWh per day.
...
TOTAL_POWER_CURVE_PER_DAY = [0 for i in range(1440)] # create empty total power profile global variable
...
coffemaker_1 = ["07:00", "15:00", coffemaker_profile_1]
...
for hour in tqdm.tqdm(range(hour_start,hour_end+1)):
    ...
    for minute in range(0, 60, interval_minutes): # was (0,59+1)
        ...
        # #plt.show()
        # plt.close() # we need that here, because we could get A LOT of figures opened
        # counter = counter + 1
...
# let's plot the optimized power curve of one day.....
plt.figure(figsize=(15,9))    
plt.step(day_minutes, TOTAL_POWER_CURVE_PER_DAY, 'k-')
